...
function funky (element, index, array) {
  return element % 2 === 0;
}
let evenNumbers  = mixedNumbers.filter(funky)
...
function taxes (element, index, arr) {
  return element * 1.07
}
...
function pop(total, num) {
  return total + num
}
let totalPopulation  = populations.reduce(pop)
...
function combatPower(element, index) {
  return element.CP > 200;
}
let myStrongest  = monstersInYourPocket.filter(combatPower)
...
function grandTotals(element){
  return element.price * (element.tax + 1)
}

let orderTotals = orders.map(grandTotals)
...
function bobbyTotal (num, total) {
return total + num
}

function bobbyFilter (element) {
  return element.owner === "Bob"
}

function bobbyMap (element) {
  return element.price
}

let bobsTotal =  purchases.filter(bobbyFilter)
.map(bobbyMap)
.reduce(bobbyTotal)
